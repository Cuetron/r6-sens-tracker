...
export default async function RegionTeams({ params }: { params: { region: string } }) {
  ...
  // Fetch only the team names for players in this specific region
  const { data, error } = await supabase
    .from('Players')
    .select('team')
    .ilike('region', decodedRegion); // ilike handles case insensitivity 
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white font-sans pb-32">
      ...
      <main className="max-w-6xl mx-auto px-12">
        <div className="mb-4">
          <Link href="/regions" className="text-[#5ce1e6] hover:text-white uppercase tracking-widest text-sm font-bold">← Back to Regions</Link>
        </div>
